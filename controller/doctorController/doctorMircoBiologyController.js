...
const getAllTests = async (req, res) => {
    ...
    try {
        ...
        if(req.user.module?.toLowerCase() !== "microbilogy" || req.user.role?.toLowerCase() !== "doctor") {
            return res.status(403).json({ message: "Access denied. Only Microbiology doctors can access this resource." });
        }
      ...
      include: [
        ...
        },
        {
          ...
          where: {
            department: "MICROBILOGY", // <- Must match DB spelling
          },
    ...
    return res.status(200).json({
      message: "Test Details of Microbilogy been fetched successfully",
      data: groupedResults,
    });
    } catch (error) {
